...
class GreatDealsSeeAllPage extends StatefulWidget {
  const GreatDealsSeeAllPage({super.key});

  @override
  State<GreatDealsSeeAllPage> createState() => _GreatDealsSeeAllPageState();
}

class _GreatDealsSeeAllPageState extends State<GreatDealsSeeAllPage> {
    ...
    return FutureBuilder<void>(
      future: (searchQuery) {
        return seeAllPageProvider
            .fetchGreatDealRestaurants(_textEditingController.text);
      ...
      builder: (BuildContext context, AsyncSnapshot<void> snapshot) {
        if (snapshot.hasError) {
          ...
          return Scaffold(
              appBar: AppBar(),
              body: const Center(child: CircularProgressIndicator()));
        } else if (seeAllPageProvider.greatDealDocs.isEmpty) {
          return Scaffold(
            appBar: AppBar(
              title: const Text('알투레 Great딜s'),
            ),
            body: const Center(
              child: Text(
                '등록된 음식점이 없습니다.',
                ...
                child: Scaffold(
                  ...
                  appBar: AppBar(
                    ...
                    title: AnimatedSearchBar(
                      ...
                      onChanged: (value) {
                        (searchQuery) {
                          return seeAllPageProvider.fetchGreatDealRestaurants(
                              _textEditingController.text);
                        }(value);
                      },
