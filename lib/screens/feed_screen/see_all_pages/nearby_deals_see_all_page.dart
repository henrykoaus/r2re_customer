...
class NearbyDealsSeeAllPage extends StatefulWidget {
  const NearbyDealsSeeAllPage({super.key});

  @override
  State<NearbyDealsSeeAllPage> createState() => _NearbyDealsSeeAllPageState();
}

class _NearbyDealsSeeAllPageState extends State<NearbyDealsSeeAllPage> {
    ...
    return FutureBuilder<void>(
      future: (searchQuery) {
        return seeAllPageProvider
            .fetchNearbyRestaurants(_textEditingController.text);
      ...
      builder: (BuildContext context, AsyncSnapshot<void> snapshot) {
        if (snapshot.hasError) {
          ...
          return Scaffold(
              appBar: AppBar(),
              body: const Center(child: CircularProgressIndicator()));
        } else if (seeAllPageProvider.nearbyDocs.isEmpty) {
          return Scaffold(
            appBar: AppBar(
              title: const Text('알투레 내주변 딜s'),
            ),
            body: const Center(
              child: Text(
                '등록된 음식점이 없습니다.',
                ...
                child: Scaffold(
                  ...
                  appBar: AppBar(
                    ...
                    title: AnimatedSearchBar(
                      ...
                      onChanged: (value) {
                        (searchQuery) {
                          return seeAllPageProvider.fetchNearbyRestaurants(
                              _textEditingController.text);
                        }(value);
                      },
