...
class PopularDealsSeeAllPage extends StatefulWidget {
  const PopularDealsSeeAllPage({super.key});

  @override
  State<PopularDealsSeeAllPage> createState() => _PopularDealsSeeAllPageState();
}

class _PopularDealsSeeAllPageState extends State<PopularDealsSeeAllPage> {
    ...
    return FutureBuilder<void>(
      future: (searchQuery) {
        return seeAllPageProvider
            .fetchPopularRestaurants(_textEditingController.text);
      ...
      builder: (BuildContext context, AsyncSnapshot<void> snapshot) {
        if (snapshot.hasError) {
          ...
          return Scaffold(
              appBar: AppBar(),
              body: const Center(child: CircularProgressIndicator()));
        } else if (seeAllPageProvider.popularDocs.isEmpty) {
          return Scaffold(
            appBar: AppBar(
              title: const Text('알투레 인기딜s'),
            ),
            body: const Center(
              child: Text(
                '등록된 음식점이 없습니다.',
                ...
                child: Scaffold(
                  ...
                  appBar: AppBar(
                    ...
                    title: AnimatedSearchBar(
                      ...
                      onChanged: (value) {
                        (searchQuery) {
                          return seeAllPageProvider.fetchPopularRestaurants(
                              _textEditingController.text);
                        }(value);
                      },
